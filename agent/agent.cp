...
//FIX-ME
//DECOUPLE
//Depends too much on Simulation
...
agent* agent::getfreeagent(gstage* stage)
{
    //DECOUPLE
...
Energy agent::damage(const Energy &e, bool nullMode)
{
    ...
    double scaleFactor = 1.0;
            ...
            //DECOUPLE
            /* fSimulation->fLowPopulationAdvantageFactor
						 ...
						 agent* carrier )
{
    ...
    populationEnergyPenalty = 1.0 * 0.5 * (agent::config.maxMaxEnergy + agent::config.minMaxEnergy);
#else
    //FIX-ME
    //DECOUPLE
    //populationEnergyPenalty = fSimulation->fPopulationPenaltyFraction * fMaxEnergy.mean();
    populationEnergyPenalty = 1.0 * fMaxEnergy.mean();
#endif
    ...
    //DECOUPLE
    /*
     fSimulation->fLowPopulationAdvantageFactor			// if ApplyLowPopulationAdvantage True
...
void agent::PickupObject( gobject* o )
{
    ...
    //FIX-ME
    //DECOUPLE
    //logs->postEvent( CarryEvent(this, CarryEvent::Pickup, o) );
}
...
void agent::DropMostRecent( void )
{
    ...
    //FIX-ME
    //DECOUPLE
    //logs->postEvent( CarryEvent(this, CarryEvent::DropRecent, o) );
}
...
void agent::DropObject( gobject* o )
{
    ...
    //FIX-ME
    //DECOUPLE
    //logs->postEvent( CarryEvent(this, CarryEvent::DropObject, o) );
}
